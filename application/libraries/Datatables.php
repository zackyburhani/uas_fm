 ...
 * @tweakAlotBy    herry_tri@yahoo.com
 */
class Datatables
{
    /**
    ...
    protected function get_filtering()
    {
        ...
        $sSearch = $this->ci->input->post('sSearch',TRUE);
        ...
        if($sSearch != '')
            for($i = 0; $i < count($mColArray); $i++){
                ...
                if($this->ci->input->post('bSearchable_' . $i) == 'true' && in_array($mColArray[$i], $columns)){
                    ...
                    $sWhere .= $this->select[$mColArray[$i]] . " LIKE '%" . $this->ci->db->escape_str($sSearch) . "%' OR ";
                    //$this->ci->db->like($this->select[$mColArray[$i]], $sSearch);
                }
            }
